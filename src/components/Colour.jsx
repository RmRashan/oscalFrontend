...
const Colour = () => {
    ...
    const handleSumbit = async () => {
        if (!Colourinput == "") {
            ...
            setloading(true)
            const res = await axios.post(`${APIURL}colour/create`, dataToSend, {
                headers: {
    ...
    return (
        <>
            ...
            <div className="min-w-[300px] relative max-w-[500px] flex justify-center flex-col items-center w-full  border p-3 rounded-2xl">
                ...
                <label className="absolute -top-4 left-5 text-[20px] font-semibold bg-white">Colour</label>
                                ...
                                <TableRow key={item.id}>
                                    ...
                                    <TableCell>
                                        <div className="relative flex items-center justify-center gap-2">
                                            ...
                                            <EditeColour key={item.id} id={item.id} name={item.name} />
                                            <Tooltip color="danger" content="Delete Colour">
                                                <span className="text-lg text-default-400 cursor-pointer active:opacity-50">

                                                    <DeleteColour key={item.id} id={item.id} name={item.name} />
                                                </span>
...
const DeleteColour = ({ id, name }) => {
    ...
    const handleDelete = async (id) => {
        const res = await axios.post(`${APIURL}colour/delete/${id}`)
        ...
        <>
            <Modal placement="top" isOpen={isOpen} onOpenChange={onOpenChange}>
                <ModalContent>
                    ...
                    <ModalHeader className="flex flex-col gap-1">Delete Colour</ModalHeader>
                    <ModalBody>

                        <h3 className="text-center text-[20px]">Do you want to delete this Colour? ?</h3>
...
const EditeColour = ({ id, name }) => {
    ...
    const handleUpdate = async (id) => {
        if (!distributorinput == "") {
            ...
            setloading(true)
            const res = await axios.post(`${APIURL}colour/update/${id}`, dataToSend, {
                headers: {
                    'Authorization': 'Bearer your-token',
                    'Content-Type': 'application/json'
                }
        ...
        <>
            ...
            <Modal placement="top" isOpen={isOpen} onOpenChange={onOpenChange}>
                <ModalContent>
                    ...
                    <ModalHeader className="flex flex-col gap-1">Edite Colour</ModalHeader>
                    <ModalBody>
